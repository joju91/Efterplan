...
| T132 | weekly-report.yml committar inte roadmap.md: Actions-workflow (`.github/workflows/weekly-report.yml`) gör `git add "$FILE"` för enbart veckorapporten — roadmap-uppdateringar och GitHub Issues-skapande saknas i det automatiserade flödet. Utöka weekly-report.mjs eller lägg till steg i yml för att inkludera roadmap.md vid ändringar. Fil: .github/workflows/weekly-report.yml. Åtgärdat: GitHub Issues-skapande fanns redan (steget "Open GitHub Issue with report"). Kvarvarande hål var just `git add "$FILE"` — staga nu även `roadmap.md` i samma steg (`git add "$FILE" roadmap.md`), så om roadmap.md ändras under samma körning (t.ex. av ett framtida steg i weekly-report.mjs) committas det tillsammans med veckorapporten istället för att tyst falla bort. `git diff --cached --quiet` täcker redan "ingen ändring alls"-fallet för båda filerna. `weekly-report.mjs` självt skriver inte till roadmap.md idag (bara läser status för sammanräkning) — att bygga automatisk ticket-generering dit är en separat, större feature och ingick inte i denna fix. | 2026-06-08 | Fas 12 | Veckorapport | 🟡 | Dev | ✔ |
