...
| T251 | **`security-review` på `design-pass`.** Branchen grenades från `main` och innehöll inte T244-koden, så ingen säkerhetsgranskning kördes i genomgången. **Kört 2026-09-14** mot mergad PR #85 (`a0cc618^..f2830ca`), fokus `app.js`, `scripts/swap-analytics.mjs`, `scripts/add-skip-link.mjs`, bopp-rader + `dodsannons.html`-generatorn. Inga fynd: `resumePlan()`/`track()` rör bara egen `localStorage`, bopp-inputs (`varde`/`belopp`) fick `_esc()` tillagt (fix, inte ny risk), dödsannons-generatorn skriver med `textContent` (ingen XSS), build-scripten körs lokalt utan användarindata. | 2026-09-02 | Fas 12 | Designgenomgång | 🟡 | Dev/Sec | ✔ |
...
| ID | Task | Date | Phase | Source | Priority | Type | Status |
|----|------|------|-------|--------|----------|------|--------|
| T258 | `vad-gora-nar-nagon-dor.html` saknas i Lighthouse CI URL-lista — prioritet 0.9 i `sitemap.xml` (näst viktigaste SEO-sida, "vad gör man när någon dör"), men täcks ej av automatisk prestanda- och tillgänglighetskoll. **Åtgärdat:** tillagd i `urls`-listan i `.github/workflows/weekly-health.yml`. Fil: `.github/workflows/weekly-health.yml`. Issue: https://github.com/joju91/Efterplan/issues/97 | 2026-09-14 | Fas 12 | Veckorapport | 🟠 | Dev/SEO | ✔ |
| T259 | `gratis-checklista-abonnemang.html` saknas i Lighthouse CI — Google Ads-landningssida (betald trafik planeras dit, trycktestade 2026-09-10), men täcks ej av automatisk prestandakoll. En prestandaregression kostar direkt i Quality Score/CPC. **Åtgärdat:** tillagd i `urls`-listan i `.github/workflows/weekly-health.yml`. Fil: `.github/workflows/weekly-health.yml`. Issue: https://github.com/joju91/Efterplan/issues/98 | 2026-09-14 | Fas 12 | Veckorapport | 🟠 | Dev/SEO | ✔ |
