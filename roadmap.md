...
| ID | Task | Date | Phase | Source | Priority | Type | Status |
|----|------|------|-------|--------|----------|------|--------|
| T165 | `index.html:924` — "Dokumenten lämnar aldrig din enhet — ingen AI-tjänst sparar bilden" är konkret felaktigt, verifierat mot `api/categorize-document.js`: bilden skickas via `fetch` till `https://api.anthropic.com/v1/messages`. Skriv om till att korrekt beskriva att bilden skickas till Anthropics API för kategorisering, inte lagras permanent (om det stämmer — verifiera Anthropics data-policy för API-anrop innan den formuleringen används). | 2026-08-12 | Fas 19 | ChatGPT-audit | 🔴 | Content | ☐ |
| T166 | `index.html:173`, `:178` + samma formulering i `checklista-dodsbo.html:99`, `dodsbo-checklista-7-dagar.html:100`, `vad-gora-nar-nagon-dor.html:99` — "begravningsbyrån ... sköter registreringen hos Skatteverket" är missvisande; Skatteverket registrerar dödsfallet via läkare/sjukhusets dödsbevis, inte begravningsbyrån. Byt till t.ex. "hjälper till med det praktiska kring begravningen och guidar dig vidare". Repeteras på 5 sidor. | 2026-08-12 | Fas 19 | ChatGPT-audit | 🔴 | Content | ☐ |
| T167 | `index.html:209` — "Efterlevandepension kan utbetalas till make, maka, sambo och barn under 20 år" är för brett; sambo har villkorad rätt (kräver t.ex. gemensamt barn eller tidigare äktenskap). Skriv om till "Efterlevande make, maka eller i vissa fall sambo kan ha rätt till omställningspension. Barn kan ha rätt till barnpension och efterlevandestöd." Kontrollera samtidigt att `efterlevandepension.html` (den dedikerade guiden) redan är korrekt nyanserad. | 2026-08-12 | Fas 19 | ChatGPT-audit | 🔴 | Content | ☐ |
| T168 | `index.html:157` — "Inga formulär, ingen registrering" överdriver; onboardingen har 4 steg med fält (relation, situation, namn/datum/personnummer). Byt till "Ingen registrering krävs för att komma igång" — stämmer faktiskt med produkten. | 2026-08-12 | Fas 19 | ChatGPT-audit | 🟠 | Content | ☐ |
| T169 | Förtydliga gräns mellan gratis och betalt (49 kr) tidigare i flödet — idag framgår det inte tydligt vad som ingår innan man investerat tid. Konkret förslag: en "Gratis: checklista, plan, PDF, bouppteckningsöversikt / 49 kr: obegränsade brev, fullmakt, dödsannons"-lista synlig innan onboarding. | 2026-08-12 | Fas 19 | ChatGPT-audit | 🟠 | Conversion | ☐ |
| T170 | Positionera Efterplan tydligare mot Efterlevandeguiden (Pensionsmyndighetens/myndigheternas egen tjänst) — inte konkurrera om informationsauktoritet, utan äga "myndigheterna säger vad som gäller, Efterplan hjälper dig göra det". Content-/copy-arbete, ingen kod. | 2026-08-12 | Fas 19 | ChatGPT-audit | 🟡 | Content | ☐ |
| T171 | SEO-guidesidorna (vad-gora-nar-nagon-dor, bouppteckning-guide, arvskifte-guide m.fl.) saknar en enhetlig CTA tillbaka till produkten i slutet av artikeln. Lägg till samma återkommande rad: "Vill du slippa göra checklistan själv? Skapa din personliga plan på Efterplan." | 2026-08-12 | Fas 19 | ChatGPT-audit | 🟡 | SEO | ☐ |
| T172 | Audit-item om cache/gamla versioner (149 kr, 6 onboarding-steg synligt i en äldre crawl) — troligen redan löst: T128 (sitemap lastmod, PR #53) och prisfixen från T032/T033 (denna session) täcker detta. Verifiera ändå att inget CDN/edge-cache serverar gammalt innehåll. | 2026-08-12 | Fas 19 | ChatGPT-audit | 🟢 | Infra | ☐ |
| T173 | `index.html:232-243` (`#section-integritet`) redan inaktuell, oberoende av ChatGPT-audit — hittades av en design-agent under research för delningsfunktionen. "Efterplan lagrar all information lokalt... ingenting skickas till någon server" stämmer inte längre för inloggade användare (Supabase-synk finns redan). Dessutom: `saveState()` (app.js:2415-2419) lägger tillbaka `personnr` i det som sparas/synkas, trots att `getShareableState()` (app.js:2355-2373) medvetet exkluderar det — så personnummer synkas idag till Supabase för inloggade användare, vilket "Personnummer delas aldrig"-löftet inte tydliggör gäller själva delning, inte kontosynk. Skriv om integritetstexten till att korrekt skilja mellan lokal-only (ej inloggad) och synkat till Supabase (inloggad). | 2026-08-12 | Fas 19 | Session (delnings-research) | 🔴 | Content | ☐ |
...
| T177 | Delning (läsbar länk, krypterad klientsidan — ny modell enligt T141, inte den skrotade `share_tokens`-modellen). Fullt designad (AES-GCM/Web Crypto, nyckel bara i URL-fragment, ny `shared_plans`-tabell + två SECURITY DEFINER-RPC:er eftersom fullständig plandata inte får plats i en URL). Ej byggd — egen session pga omfattning. | 2026-08-12 | Fas 20 | Session | 🟡 | Feature | ☐ |
| T178 | Deadline-mejl, insamlingsdelen (samtyckes-UI + ny `reminder_optins`-tabell + `api/subscribe-reminder.js`). Inget faktiskt mejlutskick i denna omgång — kräver separat val av e-postleverantör + cron. Fullt designad. Ej byggd. | 2026-08-12 | Fas 20 | Session | 🟡 | Feature | ☐ |
