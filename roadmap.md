...
| T139 | Brev till hyresvärd (uppsägning hyresrätt vid dödsfall) — guide finns (tomma-dodsbo.html), inget brev. Löst: ny brevmall i samma stil som bank/försäkring (`generateHyresvard()` i app.js), knapp visas bara om `state.hyresratt` är satt. Under arbetet upptäcktes att `getRelationLabel()` var trasig sedan tidigare (nycklarna matchade aldrig de faktiska `state.relation`-värdena, t.ex. `'partner'` istället för `'make'`) — relation-fältet i ALLA brev (bank, försäkring, fullmakt m.fl.) föll alltid tillbaka på tomt. Fixat i samma commit. | 2026-07-18 | Fas 12 | Session | 🟢 | Content | ✔ |
| T140 | Brev till Pensionsmyndigheten för efterlevandepension — guide finns (efterlevandepension.html), inget brev. Löst: ny brevmall (`generatePension()` i app.js), knapp visas bara för `state.relation === 'make'` (samma trigger som omställningspension/make_pension-uppgifterna). | 2026-07-18 | Fas 12 | Session | 🟢 | Content | ✔ |
...
| T230 (B9) | Fler brevmallar: hyresvärd (T139), Pensionsmyndigheten (T140), arvskiftesavtal-generator (T138) — var och en är en egen liten funktion, görs bäst separat. T139/T140 klara (2026-08-25). Kvarstår: T138 (arvskiftesavtal-generator). | 2026-08-14 | Fas 26 | `bygg-ett-helt-automatiskt-iridescent-peach.md` | 🟢 | Dev | ⧖ |
...
| T244 | Security headers saknas i `vercel.json`: X-Frame-Options, X-Content-Type-Options, Referrer-Policy och HSTS (`Strict-Transport-Security`) är inte satta för `efterplan.se`. Minsta åtgärd: lägg till ett global `headers`-block i `vercel.json` med `X-Frame-Options: SAMEORIGIN`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`. HSTS sätts automatiskt av Vercel men bör verifieras. Fil: `vercel.json`. Löst: globalt `headers`-block tillagt (2026-08-25). HSTS sätts av Vercel automatiskt för alla domäner med giltigt SSL-cert — inte verifierbart härifrån (kräver att titta på faktiska response-headers från produktion), lämnas som uppföljning om Owner vill dubbelkolla. | 2026-08-17 | Fas 12 | Veckorapport | 🟡 | Dev/Sec | ✔ |
...
# 🔍 SESSION — 2026-08-25: Stripe-testläge, blockerad på credentials
💡 Uppföljning/konkretisering av T164 (Fas 18, fortfarande öppen — felfallen: nekat kort, avbruten betalning, dubbel-webhook m.m.). Owner bad om att testa betalningsflödet mot Stripe **test-läge** istället för produktion, för att slippa köra riktiga live-betalningar. Research visade att `api/create-checkout.js`, `api/stripe-webhook.js`, `api/verify-checkout.js` och `api/_lib.js` redan är helt generiska — de läser `STRIPE_SECRET_KEY`/`STRIPE_PRICE_ID`/`STRIPE_WEBHOOK_SECRET` enbart från miljövariabler, ingen hårdkodning och ingen live/test-gren i koden. Ingen kodändring behövs. Men sessionen hade inga Stripe-credentials alls tillgängliga (ingen `.env.local`, inga miljövariabler, ingen Stripe/Vercel-CLI) — Owner bekräftade att han inte har åtkomst till nycklarna just nu och bad om att detta läggs som prioriterad roadmap-post istället för att köras direkt.

| ID | Task | Date | Phase | Source | Priority | Type | Status |
|----|------|------|-------|--------|----------|------|--------|
| T248 | Kör Stripe-checkout-grundflödet i test-läge (T164:s felfall tas i separat runda). Krävs innan det kan köras: (1) en test-läges secret key (`sk_test_...`) från Stripe Dashboard (test-läge-toggeln uppe till höger), (2) ett test-pris — det finns redan en tom test-produkt `prod_V5929Jn2L7WZ6E` ("Efterplan Premium (TEST QA)") från en tidigare pausad session, men den saknar pris; skapa ett (49 kr) på den, antingen manuellt i Dashboard eller via `stripe prices create` (blockerades av auto-klassificeraren i en tidigare bakgrundssession trots test-läge — värt att pröva igen interaktivt), (3) en test-webhook-secret (`whsec_test_...`) om webhook-steget ska köras. Kör lokalt via `vercel dev` + en `.env.local` (redan `.gitignore`:ad, committa aldrig) med dessa test-värden — sätt INTE test-nycklar i Vercels Production-scope (de riktiga `STRIPE_SECRET_KEY`/`STRIPE_PRICE_ID` i Production pekar mot live-läge och ska förbli så). Flöde att verifiera: `create-checkout` → öppna Stripe-checkouten → betala med testkort `4242 4242 4242 4242` → `verify-checkout` → kontrollera att `purchases`-raden i Supabase får `livemode: false`. Se `.claude/skills/keys/SKILL.md` för hur `STRIPE_SECRET_KEY` normalt roteras (samma flöde, men peka mot en lokal `.env.local` istället för Vercel). | 2026-08-25 | Fas 18 | Session | 🔴 | QA | ☐ |
