...
| ID | Task | Date | Phase | Source | Priority | Type | Status |
|----|------|------|-------|--------|----------|------|--------|
| T243 | Performance: Lighthouse CI mätte perf 0.76 (budget ≥0.80), LCP 3565ms (budget ≤2500ms), TBT 305ms (budget ≤300ms) — alla warn-nivå, över budget men fäller inte bygget. Observerades under T225-testkörning 2026-08-16. Utred och åtgärda: identifiera vilka resurser som orsakar LCP och TBT via Lighthouse-artefakt i Actions, optimera render-blockerande JS/CSS i `app.js`/`style.css`. Filer: `app.js`, `style.css`, `.lighthouserc.json`. Delvis åtgärdat 2026-08-27: rotorsak till LCP var att `style.css`/`style-tokens.css` (~88.5KB) och typsnitts-preconnect/preload låg ~110 rader in i `<head>` (efter meta/OG-taggar + 4 JSON-LD-block), vilket försenade när webbläsarens preload-scanner ens upptäckte dem. Flyttade båda blocken till direkt efter `<meta charset>`/`viewport`/`title`/`description`/canonical, före OG-taggar och JSON-LD. Ingen bild orsakar LCP (inga `<img>` ovanför falden) — LCP-elementet är `<h1 class="landing-headline">` i Fraunces. Fonterna använde redan icke-blockerande preload-swap och båda JS-bundlarna hade redan `defer`, så ingen ytterligare ändring gjordes där. **Kvarstår:** full effekt kräver även att T223 (self-hostade fonter, PR #76, ej mergad) mergas — tar bort en extern round-trip till fonts.gstatic.com i samma kritiska kedja. TBT (152KB oparsad `app.js`) hänger delvis ihop med T224 (minifiering, medvetet nedprioriterad) — omvärderas tillsammans om Lighthouse fortfarande visar rött efter denna fix + T223-merge. Critical-CSS-inlining/JS-kodsplittring gjordes medvetet inte i denna omgång (större refaktorering, egen framtida ticket vid behov). Fil: `index.html`. | 2026-08-17 | Fas 12 | Veckorapport | 🟠 | Dev/Perf | ✔ |
