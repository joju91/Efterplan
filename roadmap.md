...
| T147 | Lagring: localStorage för alla, Supabase Storage-synk för inloggade/premium (återanvänder auth från T051–T053). **Ej byggd denna omgång** — localStorage-delen klar (`efterplan_documents`, testad över reload), Supabase Storage-synken är en egen, större integration (ny bucket + policies) som medvetet sparades till en egen körning. Byggd 2026-08-27: ny `public.documents`-tabell (metadata: namn/kategori/datum/flagga/hash/storage_path, unik på `(user_id, client_id)`) + privat Storage-bucket `documents` (5MB-gräns, image/jpeg|png|webp), RLS på både tabell och `storage.objects` (folder-baserad ägarkontroll `(storage.foldername(name))[1] = auth.uid()::text`, samma mönster som Supabase egen dokumentation rekommenderar). Applicerat direkt mot produktions-Supabase via MCP (`apply_migration`), verifierat med `list_tables`/`get_advisors` — inga nya säkerhetsvarningar utöver en mutable-search-path-lint som fixades direkt. Base64-fotot går ALDRIG genom `plans.state_json` (skulle bli extremt ineffektivt) — egen väg: `dataUrlToBlob()` → `client.storage.from('documents').upload()`, bara metadata + `storage_path` i Postgres-raden. Nytt event-baserat kopplingsmönster i `app.js`↔`supabase-client.js` (samma stil som befintlig `efterplan:state-changed`): `saveDocuments()` sänder `efterplan:documents-changed` (debounce 2s, samma mönster som `syncToSupabase`), `deleteDocument()` sänder ett separat explicit `efterplan:document-deleted` (medvetet INTE en diff mot hela listan — en diff hade kunnat radera dokument som bara ännu inte hunnit hydreras ner på en ny enhet om en synk triggas innan hydreringen är klar; explicit borttagning är race-fri). Hydrering vid inloggning (`hydrateDocumentsFromRemote()`, kopplad i både `initSupabase()` och `handleAuthChange('SIGNED_IN')`): hämtar användarens rader, genererar 24h signerade URL:er för foton, skriver bara in dokument som saknas lokalt (matchat på `client_id`), och `app.js` lyssnar på ett nytt `efterplan:documents-hydrated`-event för att slå in dem i `state.documents`/rendera om utan sidladdning. localStorage förblir source-of-truth för icke-inloggade/offline — helt oförändrat beteende där. Känd begränsning (dokumenterad, inte byggd): signerade URL:er för hydrerade foton förnyas inte automatiskt om de går ut mitt i en lång session (24h bör räcka i praktiken); lokalt skapade dokument bär ingen `_storagePath`-flagga så deras foto laddas upp på nytt (harmlöst, `upsert:true`) en gång per sidladdning istället för bara en gång per dokuments livstid — en bandbreddsoptimering, inte en korrekthetsbugg, medvetet lämnad utanför scope. Verifierat: `node --check` på båda filerna, en fristående Node-test av `dataUrlToBlob()`-roundtripen (base64 → Blob → tillbaka, byte-för-byte identiskt), samt `mcp__Supabase__get_advisors` efter migrationen. Filer: `supabase-client.js`, `app.js`, `supabase/schema.sql` (dokumentation av samma migration som applicerades mot produktion). | 2026-08-11 | Fas 13 | Session | 🟡 | Dev | ✔ |
