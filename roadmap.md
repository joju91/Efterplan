...
| T222 (B1) | Fler content-luckor: `internationellt-arv.html`, `arvsavstaende.html`, `gavobrev.html`, `aktenskapsforord.html` som egna sidor (idag bara nämnda i app-logik/tickets, ingen SEO-landningssida). Åtgärdat: alla fyra sidor byggda med samma mall som de befintliga guiderna (canonical, meta, Article+HowTo+FAQPage+BreadcrumbList JSON-LD, `seo-nav`/`seo-article`/`seo-footer`). `app.js`: de två checklista-punkter som redan refererade ämnena men saknade länk (`utland_juridik`, `aktemanskapsforord`) fick nu en `resources`-post som pekar till respektive ny guide. Interlänkat från befintlig prosa som redan nämnde ämnena utan länk: `laglott.html` (gåvobrev, arvsavstående, internationellt arv), `sarkullbarn.html` (äktenskapsförord, arvsavstående) och `sambo-arv.html` (äktenskapsförord som kontrast till samboavtal) — plus footer-länkar på alla fyra nya sidor och de sidor som nu länkar in. Verifierat: alla JSON-LD-block är giltig JSON, HTML-taggar balanserade, inga brutna interna länkar, `app.js` syntax-checkad. Sitemap.xml kräver ingen manuell ändring (auto-genereras av `update-sitemap.yml` vid push till main). Filer: `aktenskapsforord.html`, `gavobrev.html`, `arvsavstaende.html`, `internationellt-arv.html`, `app.js`, `laglott.html`, `sarkullbarn.html`, `sambo-arv.html`. | 2026-08-14 | Fas 26 | `bygg-ett-helt-automatiskt-iridescent-peach.md` | 🟢 | SEO | ✔ |
