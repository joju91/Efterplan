...
| T139 | Brev till hyresvärd (uppsägning hyresrätt vid dödsfall) — guide finns (tomma-dodsbo.html), inget brev. Byggd 2026-08-27: `generateHyresvard()` + `doc-form-hyresvard`, samma mönster som befintliga brevgeneratorer (bank/försäkring). Peka om den befintliga `hyresratt_uppsagning`-tasken (`hasDoc`) från den generiska `letter`-mallen (byggd för abonnemang, fel fält) till den nya dedikerade. Ny knapp i doc-chooser, visas villkorligt när `state.hyresratt` är sant. Gratis (inte bakom betalvägg) — matchar majoriteten av breven (bank/försäkring/uppsägning/annons), inte de två undantagen (Skatteverket/fullmakt). Verifierat med en fristående Node-test (mockad DOM): inget "undefined" läcker in i brevtexten, obligatoriska fält blockerar korrekt. Filer: `app.js`, `index.html`. | 2026-07-18 | Fas 12 | Session | 🟢 | Content | ✔ |
| T140 | Brev till Pensionsmyndigheten för efterlevandepension — guide finns (efterlevandepension.html), inget brev. Byggd 2026-08-27: `generatePensionsmyndighet()` + `doc-form-pensionsmyndighet`, med telefonmanus+checklista (samma `docPhoneScript`-mönster som bank/försäkring, eftersom myndigheten har ett telefonnummer i appen redan). Täcker både omställningspension och efterlevandepension, kopplad till den befintliga `make_pension`-tasken (`hasDoc`). Visas villkorligt när `state.relation === 'make'`. Gratis, samma resonemang som T139. Verifierat med samma fristående Node-test. Filer: `app.js`, `index.html`. | 2026-07-18 | Fas 12 | Session | 🟢 | Content | ✔ |
...
| T230 (B9) | Fler brevmallar: hyresvärd (T139), Pensionsmyndigheten (T140), arvskiftesavtal-generator (T138) — var och en är en egen liten funktion, görs bäst separat. T139 och T140 byggda 2026-08-27 (se respektive ticket). Arvskiftesavtal-generatorn (T138) kvarstår — beroende av T137 (klar 2026-08-27), men själva dokumentgeneratorn är en egen, separat leverans som inte ingick i denna omgång. | 2026-08-14 | Fas 26 | `bygg-ett-helt-automatiskt-iridescent-peach.md` | 🟢 | Dev | ☐ |
