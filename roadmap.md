...
| T223 (B2) | Self-hosta Google Fonts (Fraunces/IBM Plex Sans) istället för `fonts.googleapis.com` — tar bort en extern render-blocking request, ren perf/Core-Web-Vitals-vinst. Åtgärdat: laddade ner latin-subset (täcker svenska å/ä/ö) woff2 direkt från `fonts.gstatic.com` (Fraunces v38 normal+italic, variabel wght 400–600; IBM Plex Sans v23 normal, variabel wght 400–600 delar samma fil för 400/500/600, samt italic 400) till nytt `/fonts/`-directory, 4 filer totalt (~340KB). Nya `@font-face`-regler i `style.css` (inte separat fil — laddas redan sitewide, så alla 41 HTML-sidor får nu korrekt typografi, inte bara de 13 som tidigare länkade Google Fonts direkt). `<link rel=preconnect/preload/noscript>` mot `fonts.googleapis.com` borttaget från alla 13 sidor som hade det. `sw.js` städat (särfallet för Google Fonts-cachning i fetch-listenern var dött, `CACHE` bumpad till v17, fontfilerna precachas i `ASSETS` istället). `vercel.json` fick `Cache-Control: immutable` för `/fonts/(.*)`. `style.css?v=`-cache-bust bumpad till v5 sitewide så alla sidor hämtar ny CSS. Filer: `style.css`, `sw.js`, `vercel.json`, `/fonts/*.woff2`, 41 HTML-filer (cache-bust). | 2026-08-14 | Fas 26 | `bygg-ett-helt-automatiskt-iridescent-peach.md` | 🟢 | Dev | ✔ |
