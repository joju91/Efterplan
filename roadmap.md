...
| ID | Task | Date | Phase | Source | Priority | Type | Status |
|----|------|------|-------|--------|----------|------|--------|
| T195 | Telefonmanus i dokumentgeneratorn. Upptäckt att PR #58 (Fas 28, "telefonmanus" i mergemeddelandet) i själva verket bara rörde SEO-metadata (twitter:card) — `app.js`/`index.html` ändrades aldrig, funktionen fanns bara på pappret. Byggd på riktigt nu: ny flik ("✉ Brev" / "📞 Vad du kan säga i telefon") i `doc-result`-vyn, bara synlig för brevtyper där folk oftare ringer än skriver (bank, försäkringsbolag — Försäkringskassan/begravningsbyrå har ingen egen brevgenerator att haka på). `showDocResult()` tar nu en valfri 4:e parameter `{text, checklist}`; `switchDocMode()` togglar mellan brev/telefon och visar en checklista (personnummer, ev. kund-/försäkringsnummer, egna uppgifter). Relationsfältet är fritext (inte en begränsad lista) — manuset använder "Jag är [relation] till [avliden]" (inte "Min [relation] [avliden]", som skulle bli bakvänt oavsett vilket ord som skrivs in) så det fungerar korrekt för dotter/son/mamma/pappa/sambo/syskon/vad som helst. Testat end-to-end i browser preview (bank + försäkring, båda med olika relationsvärden, plus regressionstest att brevtyper utan telefonmanus inte visar fliken). Cache-busting `app.js`/`style.css`-versionsnummer och service worker-cachenamnet (`sw.js`) höjda i samma veva — annars hade förändringen aldrig synts för återkommande besökare. | 2026-08-13 | Fas 24 | Persona-simulering session | 🟡 | Dev/Content | ✔ |
...
| T225 (B4) | Bekräftat: `weekly-health.yml` fanns bara beskriven i `MAINTENANCE.md`, aldrig skapad som faktisk workflow — sajten hade alltså inget automatiskt skydd mot trasiga länkar eller perf/SEO-regressioner. Skapad `.github/workflows/weekly-health.yml` (broken-link-check via lychee + Lighthouse CI mot startsidan/checklista-dodsbo/bouppteckning-guide + auto-issue vid fel, samma mönster som `weekly-report.yml`) och `.lighthouserc.json` (budget: perf ≥0.8, a11y/SEO ≥0.9 error, LCP ≤2500ms/CLS ≤0.1/TBT ≤300ms warn). Testkörd upprepade gånger på riktigt via `workflow_dispatch` (inte bara läst/gissat) tills grön — tre faktiska buggar hittades och fixades under vägen: (1) `./**.html` ogiltigt glob för lychee, rätt är `./**/*.html` (samma fel fanns i MAINTENANCE.md:s eget exempel); (2) `<link rel=preconnect>` mot fonts.gstatic.com/fonts.googleapis.com utan path gav falska 404-positiv, exkluderade; (3) `budgetPath` pekar på ett annat inkompatibelt schema (Lighthouse "budgets.json") än `.lighthouserc.json`s `ci.assert.assertions` — krockade tyst utan att fälla bygget, rätt input är `configPath`. Sista körningen visade assert-steget faktiskt aktivt och mätte riktiga tal (perf 0.76, LCP 3565ms, TBT 305ms — alla över budget som `warn`, fäller inte bygget men syns i loggen; värt att titta på i en framtida perf-session). | 2026-08-16 | Fas 26 | Session | 🟡 | Infra | ✔ |
...
| T241 | **T239 var bara verifierat lokalt, inte i CI** — upptäckt när de tre formella demotesterna (skapa/ändra/ta bort en sida, enligt Owners ursprungliga prompt) kördes på riktigt. GitHub-secreten `GA4_SERVICE_ACCOUNT_JSON` var oförändrad sen 4 maj, medan en ny giltig nyckel skapats lokalt 14 augusti (T209) men aldrig synkats till GitHub — CI körde alltså hela tiden på en föråldrad nyckel utan Search Console-behörighet. Secreten uppdaterad (`gh secret set`) och hela kedjan omtestad på riktigt i CI: ny sida → sitemap uppdaterad + `Sitemap resubmitted till Google Search Console` i loggen; borttagen sida → URL försvinner ur sitemap + ny notifiering till Google. Ändrad-sida-testet gav ingen synlig diff samma dag (förväntat — `lastmod` har dagsprecision). Test-artefakter borttagna efteråt, inget kvar i produktion. | 2026-08-15 | Fas 28 | Session (formella demotester) | 🔴 | Infra/SEO | ✔ |
...
# 🔍 VECKORAPPORT-TICKETS — 2026-08-16
...
# 🔍 VECKORAPPORT-TICKETS — 2026-08-17

| ID | Task | Date | Phase | Source | Priority | Type | Status |
|----|------|------|-------|--------|----------|------|--------|
| T243 | Performance: Lighthouse CI mätte perf 0.76 (budget ≥0.80), LCP 3565ms (budget ≤2500ms), TBT 305ms (budget ≤300ms) — alla warn-nivå, över budget men fäller inte bygget. Observerades under T225-testkörning 2026-08-16. Utred och åtgärda: identifiera vilka resurser som orsakar LCP och TBT via Lighthouse-artefakt i Actions, optimera render-blockerande JS/CSS i `app.js`/`style.css`. Filer: `app.js`, `style.css`, `.lighthouserc.json`. **Delvis adresserat 2026-09-02 (PR #85 / designgenomgång):** render-blockerande Google Analytics-script borttaget från alla sidor, progress-bars → GPU-`transform`. Re-mät efter merge — se T253. | 2026-08-17 | Fas 12 | Veckorapport | 🟠 | Dev/Perf | ☐ |
...
# 🔍 SESSION — 2026-08-24: veckorapport-uppföljning (trafiktapp + GSC-fel)

| ID | Task | Date | Phase | Source | Priority | Type | Status |
|----|------|------|-------|--------|----------|------|--------|
| T246 | **GSC 403 återkommit — samma orsak som T239, ny utlösare identifierad.** 2026-08-24-rapporten fick `User does not have sufficient permission for site` från Search Console-anropet. `gh secret list` visar att `GA4_SERVICE_ACCOUNT_JSON` + `GSC_SERVICE_ACCOUNT_JSON` roterades 2026-08-23T13:59 UTC — en dag innan felet dök upp, och ingen kodändring skedde i mellantiden (verifierat med `git log`). GA4-läsning fungerar fortfarande (KPI:erna i rapporten är korrekta), så det är specifikt Search Console-behörigheten för det (nya) service-accountet som saknas — precis som när `ga4-reader@intricate-tempo-496015-a0.iam.gserviceaccount.com` behövde läggas till manuellt i T239. **Kräver Owner:** logga in på https://search.google.com/search-console/users?resource_id=sc-domain:efterplan.se, lägg till service-accountets `client_email` (finns i den JSON som klistrades in i `keys:rotate -- google` 2026-08-23, eller i Google Cloud Console → IAM → Service Accounts) som Full-användare. Kan inte göras av Claude — external Google-kontobehörighet, inget API för det. | 2026-08-24 | Fas 28 | Veckorapport-uppföljning | 🔴 | Infra/SEO | ☐ |
| T247 | **Trafiktapp 82→23 sessions (17/8 vs 24/8-rapporten), Paid Search-kanalen försvann helt (8→0).** Uteslutit trasig mätning: engagement rate liknande (50%→39%), events registreras normalt, ingen kod ändrad i mellanveckan (`git log` tomt för index.html/robots.txt/vercel.json). Nedgången är alltså verklig trafik, inte ett mätfel. Troligast: (a) en Google Ads-kampanj pausades eller slut på budget — Paid Search gick till exakt 0, eller (b) föregående veckas 82 sessions var en tillfällig topp från aktiv outreach (Flashback/Reddit/marketing-outreach-commits samma vecka) som klingat av. **Kräver Owner:** kolla Google Ads-kontot (kampanjstatus/budget) och jämför med `research/`-loggarna för outreach-timing. Kan inte verifieras vidare av Claude — inget Google Ads-API-credential i miljön, och sandlådan blockerar utgående HTTPS mot externa dashboards. | 2026-08-24 | Fas 28 | Veckorapport-uppföljning | 🟠 | Analytics/Growth | ☐ |

---

# 🎨 SESSION — 2026-09-02: designgenomgång (11 skills, branch `design-pass`, PR #85)

Strukturerad design-/kvalitetsgenomgång av hela sajten — `teach-impeccable → audit → critique → normalize → harden → clarify → typeset → arrange → optimize → onboard → polish`. Alla åtgärder verifierade i browser, en commit per steg. Rapporter i `design-pass/*.md`. **PR #85 öppen mot `main`, ej mergad.**

**Åtgärdat i PR:n:** genomskinlig sticky-nav (Critical) · accentpaletten upp till WCAG 2.2 AA (sage 58→46 %, ember 60→48 %; länktext 3,6→5,9:1, primärknapp 4,16→5,9:1; nytt `--rule-strong` 3,2:1 på fältramar) · footer-träffytor 17→39 px · skiplänk på 40 innehållssidor · aria-labels på JS-genererade bouppteckningsfält · ett token-lager (`style-tokens.css` = sanningskälla) · **Google Analytics bort från alla 41 sidor, Plausible in på alla** · `role="main"` på alla SPA-skärmar · progress-bars `width`→`transform` · "Fortsätt din plan"-CTA för återvändande · "Fylls på efterhand" (var "Fler steg visas här", visades även på fulla sektioner) · stegetiketter konsekventa · onboarding-/plan-rubriker in i Fraunces-systemet · "Så här går det till" avkortad från kort-strip till stegsekvens.

| ID | Task | Date | Phase | Source | Priority | Type | Status |
|----|------|------|-------|--------|----------|------|--------|
| T248 | **Emoji som ikonografi — systemiskt.** Efterplan använder emoji som funktionella ikoner på 25+ ställen: dokumentikoner (🏦 📰 🛡 🏛 📋 ✉ 📞), integritetsbadges (🔒 🖥 📤 👤), trafikljus (🟢 🟡 🔴), lås (🔒), kamera (📷), gnista (✨), bockar (✓), stäng (✕). Krockar med den redaktionella "inte lekfullt"-briefen (`.impeccable.md` anti-ref 3). Kräver ett ikon-beslut av Owner: (a) litet custom inline-SVG-set i Fraunces/IBM Plex-vikt, (b) Lucide/Feather via inline SVG (ingen JS-dep — repot har ingen byggkedja), eller (c) behåll bara de meningsbärande (trafikljus som CSS-prickar, `.section-dot`-mönstret finns) och ta bort resten. Sedan utbyte på alla ställen inkl. `app.js`-mallsträngar + browserverifiering. Anropsställen listade i `design-pass/11-polish.md`. Arkiv-fliken (🗂) redan åtgärdad. | 2026-09-02 | Fas 12 | Designgenomgång (`/polish`) | 🟡 | Design | ☐ |
| T249 | **Inline-stilar + `.u-*`-utility-klasser → `style.css`/tokens.** `index.html` har ~43 inline `style=`-attribut; `arvskifte-mall.html` och `bouppteckning-tidslinje.html` har egna `<style>`-block; `.u-*`-klasserna i `style.css` (~rad 2862+, `.u-border-1px-solid-ddd`/`#ddd`, `.u-border-bottom-1px-solid-eee`/`#eee` m.fl.) används i 9 innehållsfiler (auth-modal, begravning-utomlands, begravningsbyra, boutredningsman, dodsannons, dodsbo-auktion, gravsten, tomma-dodsbo, vad-gora-nar-nagon-dor). Flytta till riktiga klasser, byt `#ddd`/`#eee` mot `var(--rule)`/`var(--rule-strong)`. Mekaniskt stort över många filer — eget städuppdrag. Verifiera visuellt oförändrat i browser. | 2026-09-02 | Fas 12 | Designgenomgång (`/audit` M10) | 🟢 | Dev | ☐ |
| T250 | **Brytpunktssystem.** `style.css` har `@media` på 420, 480, 601, 720 och 1024 px, blandat `min-`/`max-width`, inga tokens — "601px" är godtyckligt. Etablera 3–4 namngivna mobile-first-brytpunkter och rätta alla queries mot dem. Fil: `style.css`. | 2026-09-02 | Fas 12 | Designgenomgång (`/audit` M8) | 🟢 | Design/Dev | ☐ |
| T251 | **`security-review` på `design-pass`.** Branchen grenades från `main` och innehåller inte T244-koden, så ingen säkerhetsgranskning kördes i genomgången. Kör `security-review` när PR #85 mergats mot `main` (eller på `t244-security-headers` separat) — särskilt `scripts/swap-analytics.mjs`, `scripts/add-skip-link.mjs` och `app.js`-ändringarna (bopp-rader, `track()`, `resumePlan()`). | 2026-09-02 | Fas 12 | Designgenomgång | 🟡 | Dev/Sec | ☐ |
| T252 | **Aktivera Plausible custom events.** `optimize`-steget bytte Google Analytics mot Plausible på alla 41 sidor. `track()` i `app.js` och de två verktygssidorna skickar nu `window.plausible('<event>', {props})` — men custom goals måste läggas till i Plausible-dashboarden (Settings → Goals) för att synas: `onboarding_start`, `plan_completed`, `doc_generated`, `bouppteckning_saved`, `free_tool_to_app_click`, `free_tool_letter_generated`, `free_tool_letter_copied`, `free_tool_checklist_updated`. **Kräver Owner** — dashboard-åtgärd, inget API. **2026-09-10:** ingår nu som steg 1 i `ADS-LAUNCH-A.md` (fem viktigaste målen listade där, klistra-in-klara). | 2026-09-02 | Fas 12 | Designgenomgång (`/optimize` M3) | 🟡 | Analytics | ☐ |
| T253 | **Re-mät Lighthouse efter merge av PR #85.** `optimize`-steget tog bort render-blockerande Google Analytics-scriptet från alla sidor och flyttade progress-bar-animationerna till GPU (`transform`) — bör förbättra T243:s siffror (perf 0.76, LCP 3565 ms, TBT 305 ms). Kör CI-Lighthouse mot `main` efter merge och uppdatera T243. | 2026-09-02 | Fas 12 | Designgenomgång (`/optimize`) | 🟢 | Dev/Perf | ☐ |
| T254 | **Mätuppställning för Google Ads — beslut + implementation. BLOCKERAR launch.** Kampanjen enligt `google-ads-underlag-2026-08.md` (4 annonsgrupper, sökord, budget 75 kr/dag) är **byggd i Google Ads men aldrig aktiverad** (Owner 2026-09-02). Underlagets konverteringsspårnings-avsnitt blev **felaktigt av PR #85**: det säger "importera events från GA4" men gtag/GA4 är borttaget — bara Plausible kvar. Aktiverar man kampanjen nu startar den helt utan konverteringssignal. Mätuppställningen måste alltså på plats **innan** kampanjen sätts live. **Beslut som krävs (Owner):** (A) Plausible-only — UTM-tagga annonser (`utm_term={keyword}`), aktivera Plausible-goals (**beror på T252**), utvärdera per landningssida + Söktermsrapporten; ingen Smart Bidding. Eller (B, rek. om budget > ~1–2 tkr/mån) lägg tillbaka **enbart** Google Ads konverteringstagg (`gtag.js` med `AW-`-ID, inte `G-`), fyr konvertering på `plan_completed` + Stripe-tacksidan (49 kr, med värde) → konverteringar/kostnad **per sökord** i Ads + Smart Bidding. **Implementation (Alternativ B) BYGGD 2026-09-02 — PR #86, avstängd:** `app.js` har `ADS_CONVERSION_ID` + två etikett-konstanter (tomma = ingen kod laddas), `captureAdClick()` (gclid + utm_* → `localStorage` vid landning, alltid), mjuk konvertering i `generatePlan()`, hård i `handlePremiumReturn()` (värde från Stripe `amount_total`, `transaction_id` = session). `api/verify-checkout.js` returnerar `amount_total`. **Återstår (Owner):** skapa två konverteringsåtgärder i Google Ads → klistra in `AW-`-ID + två etiketter i `app.js`, bumpa `?v=`, deploya. Steg-för-steg: `ADS-SETUP.md`. Aktivering sätter `_gcl_*`-cookies — nämn ev. i §"Din integritet". Alternativ A (Plausible-only) kräver ingen kodändring, bara UTM + T252. Utvärdera transaktionsnära sökord (kostnad/49 kr-köp) separat från informationssökord (kostnad/`plan_generated`; konkurrerar med egen organisk ranking). **Beslut 2026-09-10: Alternativ A** (Plausible + UTM, ingen Google-kod/cookie). Alt B (PR #86, AW-tagg) mergad till `main` men **avstängd** — sparas till om testet visar att annonser är värt att skala. Full launch-checklista klistra-in-klar: **`ADS-LAUNCH-A.md`** (bara annonsgrupp 1+3, 75 kr/dag, 2 v). Landningssidorna `arvskifte-mall.html` + `gratis-checklista-abonnemang.html` trycktestade i browser 2026-09-10 (formulär→brev→`free_tool_letter_generated`, mobil+desktop, inga fel). Återstår: Owner kör dashboard-stegen 1–4 i checklistan, sen skickas siffrorna hit för beslut skala/stoppa. | 2026-09-02 | Fas 12 | Session (Owner-fråga) | 🟠 | Analytics/Growth | ☐ |

---

# 🔍 VECKORAPPORT-TICKETS — 2026-09-07

| ID | Task | Date | Phase | Source | Priority | Type | Status |
|----|------|------|-------|--------|----------|------|--------|
| T255 | `qs` DoS-sårbarhet regression i `ga4-dashboard` — T104 markerades ✔ men `npm audit` 2026-09-07 visar att GHSA-x5fp-wj9c-mxmx (array-limit bypass via bracket-key comma parsing) + GHSA-4mjr-xmp4-gh2g (DoS via Attacker Controlled isBuffer) täcker `qs 2.2.5–6.15.3`, dvs. versionen i `ga4-dashboard` är fortfarande sårbar (1 moderate). Fix: `cd ga4-dashboard && npm audit fix`. Fil: `ga4-dashboard/package-lock.json`. Issue: https://github.com/joju91/Efterplan/issues/87 | 2026-09-07 | Fas 12 | Veckorapport | 🟡 | Dev/Sec | ☐ |
| T256 | Content-Security-Policy (CSP) header saknas i `vercel.json` — T244 lägger till X-Frame-Options m.fl. men CSP är inte med. CSP är det enskilt starkaste XSS-skyddet på klientsidan. Steg 1: lägg till en permissiv CSP (`default-src 'self'; script-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net https://plausible.io; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src https://fonts.gstatic.com; img-src 'self' data:; connect-src 'self' https://*.supabase.co https://api.stripe.com https://plausible.io`) i `vercel.json` headers-block (kan kombineras med T244). Fil: `vercel.json`. Issue: https://github.com/joju91/Efterplan/issues/88 | 2026-09-07 | Fas 12 | Veckorapport | 🟡 | Dev/Sec | ☐ |
| T257 | `dodsannons.html` saknas i Lighthouse CI URL-lista — `.github/workflows/weekly-health.yml` mäter bara `efterplan.se`, `checklista-dodsbo.html` och `bouppteckning-guide.html`. `dodsannons.html` är en SEO-prioritetssida (ny generator + prisguide) men faller utanför automatisk prestanda- och tillgänglighetsbevakning. Lägg till `https://efterplan.se/dodsannons.html` i Lighthouse-URL-listan. Fil: `.github/workflows/weekly-health.yml`. Issue: https://github.com/joju91/Efterplan/issues/89 | 2026-09-07 | Fas 12 | Veckorapport | 🟡 | Analytics/SEO | ☐ |

---

# 🏰 SESSION — 2026-09-11: MOATS — strategiska vallgravar

Genomgång på Owners begäran: konkreta, försvarbara moats att bygga in i Efterplan — inte generiska SaaS-fraser. **OBS strategisk not (bekräftad av Owner):** Efterplan ligger **före** begravningsbyrå i anhörigas flöde (sökning → Efterplan → ev. byrå), inte tvärtom — ingen av tickets nedan bygger på byråpartnerskap (redan struket, se noter 2026-04-24). Sorterat enklast → svårast att bygga. Varje ticket beskriver syfte (varför det är svårt att kopiera), arbetssätt (hur det underhålls/byggs löpande) och konstruktion (konkret teknisk lösning i den här stacken).

| ID | Task | Date | Phase | Source | Priority | Type | Status |
|----|------|------|-------|--------|----------|------|--------|
| T258 | **Moat 1 — Långlivad ärendestat.** Syfte: ett dödsbo tar 6–12 månader (bouppteckning → arvskifte); ju mer ärendet känns som *deras pågående sak* och inte en session, desto högre blir tröskeln att byta verktyg mitt i. Arbetssätt: bygg vidare på befintlig `plans`-tabell (redan finns: `user_id`, `state_json`, `updated_at`) — lägg till en explicit fasstatus istället för att bara räkna klara uppgifter. Konstruktion: (1) **Delvis gjort 2026-09-11** — "Fortsätt din plan"-knappen på landningssidan visar nu namn + "X av Y uppgifter klara" (commit `37c6da3`, `app.js:goToLanding()` + `index.html` + `style.css`). (2) **Upptäckt gap, måste fixas för att moaten ska betyda något:** `init()` i `app.js` (rad ~3353) hoppar förbi landningssidan helt när sparad state finns — skickar rakt till `screen-plan` — så knappen från (1) syns i praktiken bara i ett sällsynt edge-case (tillbaka-klick i onboarding steg 1). Antingen ändra `init()` till att visa ett "Välkommen tillbaka"-läge, eller flytta namn+progress-informationen till en synlig plats på `screen-plan` istället. (3) Ny kolumn `phase` (text: `bouppteckning`/`arvskifte`/`klar`) på `plans` i `supabase/schema.sql`, satt av samma flöde som `applyDeadlines()`/`bouppRegDatum` redan driver. | 2026-09-11 | Fas 29 | Session (moats) | 🟠 | Product/Dev | ☐ |
| T259 | **Moat 2 — Flerpersonskoordinering.** Syfte: när flera syskon/anhöriga delar samma ärende och har delat upp uppgifter blir tjänsten kollektiv egendom i familjen — ingen enskild person vill vara den som föreslår att byta verktyg. Arbetssätt: bygg vidare på befintlig `share_tokens`-modell (kind `edit` finns redan, `toggle_shared_task()`-RPC finns redan i `supabase/schema.sql`) till en riktig namngiven tilldelning istället för en anonym redigeringslänk. Konstruktion: nytt fält `assigned_to` (fritext, inget eget konto krävs) i uppgiftsobjekten i `state_json.efterplan_tasks`, eller egen kolumn på `task_completions`; ny sektion i plan-vyn ("Anna: 4 uppgifter, Erik: 2 uppgifter") som läser samma fält. Bygger direkt på T258:s datamodell. | 2026-09-11 | Fas 29 | Session (moats) | 🟡 | Product/Dev | ☐ |
| T260 | **Moat 3 — Äga sökmomentet innan byrå kontaktas.** Syfte: "vad gör jag efter ett dödsfall" har låg kommersiell konkurrens eftersom få vill optimera hårt mot sorg — den som äger organiskt förtroende här vinner sökordet över tid, och **det är just detta som gör att Efterplan hinner före byrån**, inte ett avtal med byråer (avvisat, se strategisk not 2026-04-24). Arbetssätt: fortsätt samma sida-för-sida SEO-mönster som redan fungerar (T235 gratis-checklista-abonnemang, T240 arvskifte-mall) — interaktiva verktyg, inte statiska mallar, för sökord som annars domineras av mallsajter. Bevaka effekten löpande via redan schemalagd `gsc-positions.yml` (T245). Konstruktion: inget nytt system — identifiera näst största SEO-gap (samma researchmetod som T240) och bygg nästa interaktiva sida. Ingen kod-ticket förrän ett specifikt sökordsgap är identifierat. | 2026-09-11 | Fas 29 | Session (moats) | 🟡 | SEO/Content | ☐ |
| T261 | **Moat 4 — Brevmotor kopplad till institutionsdata.** Syfte: brev som är rätt formatterade *per mottagare* (rätt adress, rätt bilagor, rätt blankettnummer för just den banken/försäkringsbolaget) går inte att kopiera genom att bara stjäla HTML — värdet sitter i kopplingen mot institutionsdatabasen (T262), som denna ticket är beroende av. Arbetssätt: när T262 finns, låt brevgenereringen i `app.js` slå upp mottagarens exakta krav istället för att använda en generisk mall. Konstruktion: ny uppslagsfunktion i `app.js` som läser `data/institutioner.json` (från T262) och fyller brevmallen med institutionsspecifik adress/bilagekrav. **Blockerad av T262.** | 2026-09-11 | Fas 29 | Session (moats) | 🟢 | Product/Dev | ☐ |
| T262 | **Moat 5 — Institutionsdatabas.** Syfte: kärndatamoaten — en verifierad databas över processen hos varje svensk bank, försäkringsbolag och myndighet är tråkig och tidskrävande att bygga, vilket är precis varför den är svår att kopiera snabbt. Arbetssätt: research institut för institut (de ~15–20 största bankerna/försäkringsbolagen i Sverige initialt), varje uppgift källhänvisad och verifierad mot institutets egen webbplats/kundtjänst — **aldrig gissad eller fabricerad data**, matchar principen "inga fejkade uppgifter" rakt av. Utökas löpande, kvalitetssäkras av T263. Konstruktion: ny strukturerad datakälla — `data/institutioner.json` (namn, typ bank/försäkring/myndighet, krav-lista, blankett-url, käll-url, `senast_verifierad`-datum) eller motsvarande Supabase-tabell `institutions` om delning mellan flera Vercel-funktioner blir aktuellt. Första leverans: 5–10 största bankerna, verifierat manuellt, inte skrapat. | 2026-09-11 | Fas 29 | Session (moats) | 🟠 | Research/Data | ☐ |
| T263 | **Moat 6 — Bevakningsrutin för juridisk/institutionell aktualitet.** Syfte: regler för bouppteckning/arvskifte och bankers/myndigheters rutiner ändras över tid — en databas som inte underhålls blir fel inom månader, och just underhållsbördan är moaten (inte en hemlighet, ett löpande jobb). Arbetssätt: kvartalsvis granskning där varje post i institutionsdatabasen (T262) stäms av mot institutets aktuella krav; poster äldre än 6 månader flaggas automatiskt. Konstruktion: `senast_verifierad`-fältet från T262 driver en enkel flaggningsregel (i UI eller ett script som listar utgångna poster); själva granskningen är manuellt arbete, inte automatiserbar utan att gissa data. **Beroende av T262.** | 2026-09-11 | Fas 29 | Session (moats) | 🟢 | Research/Process | ☐ |
| T264 | **Moat 7 — Direktintegration mot Skatteverket/Lantmäteriet.** Syfte: regulatorisk och teknisk barriär som inte går att kopiera genom att läsa koden — kräver avtal och API-ansökan, inte bara utvecklartid. Arbetssätt: **kräver Owner** — kontakt med Skatteverket (bouppteckningsdata) och/eller Lantmäteriet (lagfart/fristar) för avtal/API-åtkomst. Kan inte initieras av Claude (extern myndighetskontakt, inget API-credential finns). Konstruktion: TBD tills avtal finns — parkerad tills Owner initierat kontakt, samma mönster som T003/T004 (Bolag-tickets som väntar på Owner-åtgärd). | 2026-09-11 | Fas 29 | Session (moats) | 🟢 | Infra/Legal | ☐ |
