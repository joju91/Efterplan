...
| T248 | **Namnbyte Efterplan → Efterarbete utvärderat och avfärdat — men ordet "efterarbete" ska in i copyn.** En användarcitat idag: "jag trodde dödstillfället var värsta, jag var inte beredd på efterarbetet." Analys: 1282 träffar på "Efterplan" i 133 filer, varav 552 kopplade till domänen efterplan.se — ett fullt namnbyte innebär SEO-nollställning, Stripe/Supabase-omkonfiguration och tappad domänhistorik, utan branding-vinst (produktnamnet Efterplan äger sitt eget sökord; "efterarbete" är ett generiskt ord som redan konkurrerar med bygg-/eventbranschen och känns mer som pappersarbete än stöd). Namnet behålls. Åtgärd istället: väv in ordet "efterarbetet" i landningssidans copy för att träffa språket målgruppen själv använder. Konkreta ställen i `index.html`: `<title>` (rad 19), `og:title`/`og:description` (rad 27–28), `meta description` (rad 20), `landing-eyebrow` (rad 154), `<h1 class="landing-headline">` (rad 155). Förslag: "Du var förberedd på dödsfallet. Inte på efterarbetet." som ny rubrik/eyebrow-formulering. | 2026-09-05 | Fas 28 | Session (namnbytesanalys) | 🟡 | Content/SEO | ☐ |
