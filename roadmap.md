...
| ID | Task | Date | Phase | Source | Priority | Type | Status |
|----|------|------|-------|--------|----------|------|--------|
| T258 | **Moat 1 — Långlivad ärendestat.** Syfte: ett dödsbo tar 6–12 månader (bouppteckning → arvskifte); ju mer ärendet känns som *deras pågående sak* och inte en session, desto högre blir tröskeln att byta verktyg mitt i. Arbetssätt: bygg vidare på befintlig `plans`-tabell (redan finns: `user_id`, `state_json`, `updated_at`) — lägg till en explicit fasstatus istället för att bara räkna klara uppgifter. Konstruktion — alla tre delar gjorda 2026-09-11: (1) "Fortsätt din plan"-knappen på landningssidan visar namn + "X av Y uppgifter klara" (commit `37c6da3`). (2) Plan-vyns logga är nu klickbar tillbaka till landningssidan (commit `feb393b`) — stänger gapet där `init()` annars hoppar förbi landningssidan för återvändande användare, så knappen från (1) faktiskt går att nå. (3) Ny kolumn `plans.phase` (`bouppteckning`/`arvskifte`/`klar`) i `supabase/schema.sql`, härledd av `derivePhase()` i `supabase-client.js` ur `bouppRegDatum` + `arvskifte`-uppgiftens done-status vid varje `savePlan()` (commit `85e3bf1`). **Kräver Owner:** Supabase-projektet (`vjupkemzpnrahdsljenl`) är pausat (status INACTIVE) — väck det i dashboarden och kör `supabase/schema.sql` för att `phase`-kolumnen ska finnas live. | 2026-09-11 | Fas 29 | Session (moats) | 🟠 | Product/Dev | ⧖ |
