...
| T244 | Security headers saknas i `vercel.json`: X-Frame-Options, X-Content-Type-Options, Referrer-Policy och HSTS (`Strict-Transport-Security`) är inte satta för `efterplan.se`. **Åtgärdat 2026-09-12:** nytt globalt `headers`-block i `vercel.json` (`/(.*)`, placerat överst i arrayen) sätter `X-Frame-Options: SAMEORIGIN`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`. Krockar inte med de befintliga specifika cache-control-reglerna (Vercel slår ihop headers från alla matchande regler). HSTS sätts automatiskt av Vercel för SSL-domäner — verifiera live efter deploy med `curl -sI https://efterplan.se/`. Fil: `vercel.json`. | 2026-08-17 | Fas 12 | Veckorapport | 🟡 | Dev/Sec | ✔ |
...
| ID | Task | Date | Phase | Source | Priority | Type | Status |
|----|------|------|-------|--------|----------|------|--------|
| T255 | `qs` DoS-sårbarhet regression i `ga4-dashboard` — T104 markerades ✔ men `npm audit` 2026-09-07 visar att GHSA-x5fp-wj9c-mxmx (array-limit bypass via bracket-key comma parsing) + GHSA-4mjr-xmp4-gh2g (DoS via Attacker Controlled isBuffer) täcker `qs 2.2.5–6.15.3`, dvs. versionen i `ga4-dashboard` är fortfarande sårbar (1 moderate). **Åtgärdat 2026-09-12:** `npm audit fix` körd i `ga4-dashboard/` — `npm audit` visar nu 0 sårbarheter. Bara lockfilen ändrades (`package.json` orörd). Fil: `ga4-dashboard/package-lock.json`. Issue: https://github.com/joju91/Efterplan/issues/87 | 2026-09-07 | Fas 12 | Veckorapport | 🟡 | Dev/Sec | ✔ |
| T256 | Content-Security-Policy (CSP) header saknas i `vercel.json` — T244 lägger till X-Frame-Options m.fl. men CSP är inte med. **Åtgärdat 2026-09-12:** CSP tillagd i samma `headers`-block som T244. Verifierad mot faktisk resursanvändning (grep över alla 41 HTML-filer + `app.js`, inga iframes/workers/websockets): `script-src 'self' 'unsafe-inline' https://plausible.io https://cdn.jsdelivr.net` (jsdelivr = `jsQR`-biblioteket för räkningsskanning, `app.js:1755`), `style-src 'self' 'unsafe-inline' https://fonts.googleapis.com`, `font-src 'self' https://fonts.gstatic.com`, `img-src 'self' data:` (bildförhandsvisningar är `FileReader.readAsDataURL`, inte blob), `connect-src 'self' https://plausible.io` + Supabase-projektets URL, `frame-ancestors/base-uri/form-action 'self'`. **Begränsning:** `'unsafe-inline'` på script-src krävs — sajten har hundratals inline `onclick=`-attribut och `<script>`-block utan nonce-infrastruktur; CSP:n stoppar alltså inte inline-XSS, men stoppar exfiltrering till andra domäner, clickjacking (frame-ancestors) och base-tag-kapning. Fil: `vercel.json`. Issue: https://github.com/joju91/Efterplan/issues/88 | 2026-09-07 | Fas 12 | Veckorapport | 🟡 | Dev/Sec | ✔ |
| T257 | `dodsannons.html` saknas i Lighthouse CI URL-lista — `.github/workflows/weekly-health.yml` mäter bara `efterplan.se`, `checklista-dodsbo.html` och `bouppteckning-guide.html`. `dodsannons.html` är en SEO-prioritetssida (ny generator + prisguide) men faller utanför automatisk prestanda- och tillgänglighetsbevakning. **Åtgärdat 2026-09-12:** `https://efterplan.se/dodsannons.html` tillagd i Lighthouse-URL-listan. Fil: `.github/workflows/weekly-health.yml`. Issue: https://github.com/joju91/Efterplan/issues/89 | 2026-09-07 | Fas 12 | Veckorapport | 🟡 | Analytics/SEO | ✔ |

---

# 🔍 VECKORAPPORT-TICKETS — 2026-09-14

| ID | Task | Date | Phase | Source | Priority | Type | Status |
|----|------|------|-------|--------|----------|------|--------|
| T258 | `vad-gora-nar-nagon-dor.html` saknas i Lighthouse CI URL-lista — prioritet 0.9 i `sitemap.xml` (näst viktigaste SEO-sida, "vad gör man när någon dör"), men täcks ej av automatisk prestanda- och tillgänglighetskoll. Lägg till `https://efterplan.se/vad-gora-nar-nagon-dor.html` i `urls`-listan i `.github/workflows/weekly-health.yml`. Fil: `.github/workflows/weekly-health.yml`. Issue: https://github.com/joju91/Efterplan/issues/97 | 2026-09-14 | Fas 12 | Veckorapport | 🟠 | Dev/SEO | ☐ |
| T259 | `gratis-checklista-abonnemang.html` saknas i Lighthouse CI — Google Ads-landningssida (betald trafik planeras dit, trycktestade 2026-09-10), men täcks ej av automatisk prestandakoll. En prestandaregression kostar direkt i Quality Score/CPC. Lägg till `https://efterplan.se/gratis-checklista-abonnemang.html` i `urls`-listan i `.github/workflows/weekly-health.yml`. Fil: `.github/workflows/weekly-health.yml`. Issue: https://github.com/joju91/Efterplan/issues/98 | 2026-09-14 | Fas 12 | Veckorapport | 🟠 | Dev/SEO | ☐ |
| T260 | 101 inline `onclick=`-attribut i HTML-filer blockerar CSP-förstärkning — T256 accepterade `'unsafe-inline'` i `script-src` som nödvändig kompromiss. Inventering: 101 inline handlers + 6 `<script>`-block i ~20 filer. Flytta handlers till `addEventListener` i `app.js`; börja med `index.html` som pilot. Möjliggör därefter nonce-baserad CSP som stoppar inline XSS, inte bara exfiltrering. Filer: alla `.html`-filer + `app.js` + `vercel.json`. Issue: https://github.com/joju91/Efterplan/issues/99 | 2026-09-14 | Fas 12 | Veckorapport | 🟡 | Dev/Sec | ☐ |

---

# 🔍 VECKORAPPORT-TICKETS — 2026-09-21

| ID | Task | Date | Phase | Source | Priority | Type | Status |
|----|------|------|-------|--------|----------|------|--------|
| T261 | `package.json` engine-field felaktig — deklarerar `"node": ">=18"` men `@supabase/*` 2.112.3 (i package-lock.json) kräver `"node": ">=22.0.0"` och `iceberg-js@0.8.1` (transitiv Supabase-dep) kräver `"node": ">=20.0.0"`. Vercel:s default är Node 22 i praktiken, men npm-fältet vilseleder; uppdatera till `"node": ">=22.0.0"` i `package.json`. Fil: `package.json`. Issue: https://github.com/joju91/Efterplan/issues/103 | 2026-09-21 | Fas 12 | Veckorapport | 🟠 | Dev | ☐ |
| T262 | `package-lock.json` inaktuell — låser `@supabase/supabase-js` till 2.112.3 medan `^2.105.4` tillåter 2.116.0, och `stripe` till 22.5.0 medan `^22.1.1` tillåter 22.6.2. Kör `npm install` i repo-root och committa uppdaterad lockfil. Inga kända sårbarheter i versionsskillnaden, men lock-filen bör följa semver-intervallet. Fil: `package-lock.json`. Issue: https://github.com/joju91/Efterplan/issues/104 | 2026-09-21 | Fas 12 | Veckorapport | 🟡 | Dev | ☐ |
| T264 | **Google Ads dag 5-utvärdering (2026-09-28).** Kampanjen gick live 2026-09-23 (T254). Hämta klick + kostnad (Ads) och `free_tool_letter_generated` / `premium_activated` filtrerat på `utm_source=google` (Plausible) + *Köp 49 kr*-konverteringar (Ads). Beslut enligt `ADS-LAUNCH-A.md`: kostnad per 49 kr-köp < 50 kr → skala; många klick men inga brev → landningssidan; > 50 kr → stoppa, lägg pengarna på SEO. Städa samtidigt: gamla konverteringsåtgärden "Köp" (felkonfigurerad, primär) kan tas bort; en testkonvertering (`transaction_id=cs_test`, 49 kr) + en plan-konvertering från lokal verifiering 2026-09-23 kan synas och ska räknas bort. | 2026-09-23 | Fas 12 | Session (Ads-launch) | 🟠 | Analytics/Growth | ☐ |

---

# 📡 FAS 27 — COMMUNITY-SCAN-RUTINEN ÄR BLOCKERAD (2026-09-22)

💡 T211 (2026-08-14) skapade den återkommande molnrutinen "Community-scan efterplan.se" (`trig_01RSTqpLnTb2UapAYiYvkrFw`). Den kördes för första gången 2026-09-21 06:13 UTC och gav noll resultat — inte för att inga trådar hittades, utan för att sessionens nätverkspolicy (agent-proxy) helt nekar utgående uppkoppling till målforumen. Testat igen manuellt 2026-09-22, samma resultat.

| ID | Task | Date | Phase | Source | Priority | Type | Status |
|----|------|------|-------|--------|----------|------|--------|
| T263 | Community-scan-rutinen (T211) kan inte köra i den här sandboxen — agent-proxyn svarar `403 connect_rejected` på TCP-nivå (CONNECT-tunnel) för `reddit.com`, `flashback.org` **och** `familjeliv.se`, både via WebFetch och rå `curl`. Det är alltså inte en enskild sidas blockering utan hela domäner nekade av org-policy. Verifierat två gånger (2026-09-21 automatiskt, 2026-09-22 manuellt) med samma utfall. WebSearch (som går via ett annat backend) ger fortfarande träfflistor/snippets, men utan sidåtkomst går det inte att läsa hela tråden, bekräfta att den fortfarande är aktiv, kolla community-reglerna på plats eller se om någon redan länkat en konkurrent — att skriva svarsförslag ändå vore att gissa, vilket rutinens egna instruktion ("Läs tråden och förstå vad personen faktiskt frågar") uttryckligen kräver att man inte gör. Rutinen föll tillbaka korrekt på sin "inga nya fynd"-regel (ingen commit, ingen PR) och skickade istället en notis om blockeringen. **Kräver Owner-beslut:** (a) vitlista `reddit.com`/`flashback.org`/`familjeliv.se` i miljöns nätverkspolicy så rutinen kan läsa trådar på riktigt, (b) skriv om rutinen till att basera fynd enbart på WebSearch-snippets (snabbare men högre risk för felaktiga sammanfattningar/svarsförslag — bör i så fall flaggas tydligt i `community-watch.md`), eller (c) pausa/ta bort triggern (`trig_01RSTqpLnTb2UapAYiYvkrFw`) tills (a) är löst, så den inte fortsätter köra veckovis utan att kunna producera något. | 2026-09-22 | Fas 27 | Session | 🟠 | Infra | ☐ |
