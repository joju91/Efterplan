...
| ID | Task | Date | Phase | Source | Priority | Type | Status |
|----|------|------|-------|--------|----------|------|--------|
| T137 | Arvsfördelningslogik i bouppteckningsmodulen (utökar T126): lägg till särkullbarns rätt till direkt arvslott (med möjlighet till arvsavstående enligt 3 kap 9 § ÄB), laglottsberäkning vid testamente (halva legala arvslotten), sambo-bodelning (endast samboegendom, inte hela boet). Byggd 2026-08-27: nytt `state.civilstand` ('gift'/'sambo', progressive disclosure under "Var gift eller sambo"-kryssrutan, samma mönster som `bostadTyp`). `boppData.delbagare` fick `barnTyp` ('gemensamt'/'sarkullbarn', bara relevant för `roll==='arvinge'` — filtrerar bort syskon/föräldrar som också kan ha den rollen) + `avstarArv` (bara valbar för särkullbarn, aldrig gemensamma barn — uppskjuten rätt är automatisk enligt lag för dem). `boppData.tillgangar` fick `samboegendom` (kryssruta, syns bara vid civilstand==='sambo'). Två nya beräkningsfunktioner i `app.js`: `boppComputeSamboBodelning()` (halva samboegendomen till efterlevande sambo, lämnar gifta pars fulla-netto-beteende helt oförändrat) och `boppComputeArvsfordelning()` (arvslott per bröstarvinge, laglott = halva arvslotten, direkt utbetalning för icke-avstående särkullbarn, efterarv för resten). Ny sammanfattningssektion i bouppteckningsfliken (`#bopp-arvsfordelning`) med gul varningsruta vid testamente ("laglottsgolv, inte vad testamentet faktiskt ger", länk till `laglott.html`) och en "inte juridisk rådgivning"-rad. Nödvändig wiring-fix: `boppRender()` anropas nu även från `generatePlan()` (inte bara en gång vid `DOMContentLoaded`) eftersom civilstånd/testamente-state inte finns förrän onboarding är klar. **Uttryckligen utanför scope** (dokumenterat i koden, egna framtida tickets): full giftorättsgods-bodelning för gifta par, testamente-fördelningseditor (hör hemma i T138), arvsklass 2/3, barnbarn/istadarätt, förstärkt laglottsskydd/förskott på arv. `arvsavstaende.html` (nämnd i ursprungsticketen) finns ännu inte som egen sida — det är fortfarande T222:s öppna punkt; UI:t länkar istället till `laglott.html`/`sarkullbarn.html` som redan täcker reglerna. Verifierat med två fristående Node-testsviter (ingen browser-MCP tillgänglig i den här miljön): 21 aritmetik-assertions mot de exakta extraherade beräkningsfunktionerna (blandat hushåll, avstående, sambo-bodelning, civilstand=null-fallback, N=0, testamente-varning, avrundning enligt `laglott.html`s eget exempel) — alla gröna. Samt 6 DOM/HTML-genererings-tester (taggbalans + villkorlig synlighet för barnTyp/avstarArv/samboegendom i alla kombinationer, inklusive XSS-escape-kontroll) — alla gröna. Filer: `app.js`, `index.html`, `style.css`. | 2026-07-18 | Fas 12 | Session | 🟢 | Dev | ✔ |
