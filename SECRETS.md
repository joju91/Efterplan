...
| **Google (GA4/GSC)** | `GA4_SERVICE_ACCOUNT_JSON`, `GSC_SERVICE_ACCOUNT_JSON`, `GA4_PROPERTY_ID` | Nej ännu — ligger idag bara som GitHub Actions-secrets. Speglas nu även som Vercel-vars i `config.mjs`, men Vercel-sidan är tom tills nästa `keys:rotate -- google` fyller i den — därefter fångar `keys:sync` dem automatiskt precis som Stripe/Anthropic | console.cloud.google.com → IAM → Service Accounts — manuellt, sen `keys:rotate -- google` |
